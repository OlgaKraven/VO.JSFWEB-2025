...
## Основы web-приложения и настройка среды разработки JavaScript
...
Для выполнения лабораторной работы используется предметная область, **закреплённая за студентом на семестр** (см. перечень в ЛМС). 
[Предметные области](domain.md)
---
...
В `index.html` необходимо реализовать **простейшую статическую верстку**, отражающую предметную область.
...
4. **Ссылка на репозиторий**,прикрепляется студентом к ответу. 
